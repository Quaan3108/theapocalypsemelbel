...
async function openShopMenu(player) {
    ...
    switch (res.selection) {
        ...
        case 10: return openFavoriteMenu(player);
        case 11: return openTransactionHistoryMenu(player);
...
if (curr > prev) sym = "";
        ...
        form.button(`${item.displayName}\nPrice: $${formatNumber(curr)} ${sym}  Stock: ${formatNumber(stock)}`, item.icon);
    }
    form.button("§cBack");
...
async function openFavoriteMenu(player) {
    ...
    const allItems = [
        ...
        ...armor, ...ores, ...farm, ...food, ...dye, ...redstone, ...tools, ...miscellaneous
    ];
    ...
    const form = new ActionFormData()
        .title("§8§lFavorite Items and Block")
        .body("Select:");
